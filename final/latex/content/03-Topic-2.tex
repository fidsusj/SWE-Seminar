...
Section \autoref{sec:literature-search} documents the literature search used to find a different approach.
The two articles are then described in the sections \autoref{sec:el-attar} and \autoref{sec:longo}.
...
The following section \autoref{sec:comparison} includes the comparison of the two approaches using a synthesis matrix.
Section \autoref{sec:topic_2_conclusion} provides a Conclusion including the most important insights of the process and an assessment in which situations the approaches might be suitable.
...
An overview of the data exchange during the process is shown in figure \autoref{fig:overview-fitnesse} on the next page.
...
Table \autoref{fig:lit-search-fitnesse} on the next page provides an overview for the search-term-based literature search.
...
Figure \autoref{fig:overview-el-attar} shows the rough structure of the approach.
...
Figure \autoref{fig:use-case-mm} on the next page shows the Use Case Model for the Movie Manager application.
...
The domain model of the Movie Manager application is shown in Figure \autoref{fig:domain-mm} on the next page.
...
The HLATs for the Use Case \textit{Describe a Performer} are displayed in Table \autoref{fig:hlats-mm}.
...
The robustness diagram for the Use Case \textit{Describe a movie} is shown in figure \autoref{fig:robustness-mm} on the next page.
A robustness diagram contains the involved objects that communicate with the user.
These are called boundary objects.
An example for a boundary object is \textit{PerformerView} in figure \autoref{fig:robustness-mm}.
It also contains control-objects like \textit{RelateToMovie} in figure \autoref{fig:robustness-mm} that makes sure that a performer is related to at least one movie.
The last types of objects are the \textit{User} and the entities like \textit{Performer} or \textit{Movie} in figure \autoref{fig:robustness-mm}.
...
The ActionFixtures for the three HLATs from table \autoref{fig:hlats-mm} are displayed in the tables \autoref{fig:eats1-mm}, \autoref{fig:eats2-mm} and \autoref{fig:eats3-mm} on the next page.
...
Figure \autoref{fig:overview-longo} provides an overview over the steps of the approach.
...
The US-UID for the example is displayed in figure \autoref{fig:us-uid-mm}.
...
The data flow during the execution of acceptance tests with FitNesse and the role of the Fixture-Class in this process is described in section \autoref{sec:topic_2_intro}.
...
The resulting \textit{Fit-table} for the example is displayed in table \autoref{fig:fit-longo} on the next page.
