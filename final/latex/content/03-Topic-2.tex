...
An article that focusses on the topic of creating acceptance tests that are executable with FitNesse was provided by the supervisors of the seminar.
...
If they are the same, the test was succesful and the entry of the table receives the colour \textit{Green}.
...
\begin{itemize}
	...
	This criterium was chosen to find approaches that are specific to the subject of this chapter.
	...
	\item \textit{Criterium 2:} The article was not published before the year 2009 which is the year that the article by El-Attar and Smith was published.
...
The backward-snowballing did not result in any relevant articles due to their publishing date and hence not passing Criterium 2.
...
The involved objects and the communication between them is extracted from the Use Case desription.
...
Also missing steps or preconditions in the Use Case desription might be found.
...
To illustrate the approach the Use Case \textit{Describe a perfomer} is used.
...
This is possible because the US-UIDs created in the approach of Longo et al. are a different way to display the informations of a \textit{Fit-table} and therefore can be directly converted to a \textit{Fit-table}.
